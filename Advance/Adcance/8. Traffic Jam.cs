    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            while ((car = Console.ReadLine()) != "end")
            {
                if (car != "green")
                {
                    traficJam.Enqueue(car);
                }
                else
                {
                    if (traficJam.Count < carsPass)
                        carsPass = traficJam.Count;


                    for (int i = 0; i < carsPass; i++)
                    {
                        Console.WriteLine($"{traficJam.Dequeue()} passed!");
                        counter++;
                    }
                }
            }
...
//    When the light goes green N number of cars pass the crossroads and for each, a message "{car} passed!" is 
//    displayed. When the "end" command is given, terminate the program and display a message with the total number
//    of cars that passed the crossroads.
//Input
